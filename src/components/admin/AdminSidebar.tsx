...
const managementNavItems: NavItem[] = [
  ...
  { href: "/admin/pos", icon: MonitorSmartphone, label: "POS", exact: true },
...
export function AdminSidebar({ user }: AdminSidebarProps) {
  ...
  const navItems = user.role === "STAFF" ? staffNavItems : managementNavItems;
      ...
      >
            ...
            {!isCollapsed && (
              <div className="overflow-hidden whitespace-nowrap">
                ...
                <p className="text-[10px] text-[var(--admin-sidebar-text)] uppercase font-semibold tracking-[0.15em] mt-0.5">
                  {user.role === "STAFF" ? "Staff POS" : "RMS Portal"}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="px-4 py-4 border-t border-white/5 flex flex-col gap-2 relative">
          <button
            ...
            title={isCollapsed ? "Mo rong cot chuc nang" : "Thu gon cot chuc nang"}
          >
            {isCollapsed ? (
              <ChevronRight size={18} />
            ) : (
              <div className="flex items-center gap-2">
                <span className="opacity-70">Thu gon</span> <ChevronLeft size={16} />
              </div>
            )}
          </button>
          ...
          <button
            ...
            title={isCollapsed ? "Dang xuat tai khoan" : undefined}
          ...
          >
            ...
            {!isCollapsed && <span className="truncate">Dang xuat tai khoan</span>}
          </button>
        </div>
      </aside>
    </>
