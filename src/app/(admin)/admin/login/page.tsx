...
export default async function AdminLoginPage() {
  ...
  if (session?.user) {
    redirect(session.user.role === "STAFF" ? "/admin/pos" : "/admin");
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-jade via-jade-mid to-[#0d3326] flex items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm relative">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-white text-2xl font-semibold">Phương Anh Admin</h1>
        ...
        <p className="text-center text-white/30 text-xs mt-6">© {new Date().getFullYear()} Nhà hàng Phương Anh</p>
      </div>
    </div>
  );
}
